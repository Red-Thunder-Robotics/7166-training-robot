...
public final class AutoDriveCommands {
    ...
    public static Command driveToPose(Pose2d target) {
        ...
        return Drive.instance.runOnce(() -> {
            ...
            Drive.instance.runVelocity(new ChassisSpeeds(
                controllerX.calculate(StateMachine.odometryAndVision.getEstimatedPose().getX()),
                controllerY.calculate(StateMachine.odometryAndVision.getEstimatedPose().getY()),
                0d));
        })
        .until(() -> {
            final boolean x = controllerX.getError() < Units.inchesToMeters(1d) && controllerX.getErrorDerivative() < Units.inchesToMeters(0.5d);
            final boolean y = controllerY.getError() < Units.inchesToMeters(1d) && controllerY.getErrorDerivative() < Units.inchesToMeters(0.5d);
            return x && y;
        }))
        .finallyDo(() -> {
            controllerX.close();
