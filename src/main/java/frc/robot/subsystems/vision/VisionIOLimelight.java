...
public final class VisionIOLimelight implements VisionIO {
    public VisionIOLimelight() {
        for (int i = 0; i < limelightList.length; i++) {
            ...
            for (int port = 5800; port <= 5809; port++) PortForwarder.add(port + (i * 10), name + ".local", port);
        }
    ...
    public void updateInputs(VisionIOInputs inputs) {
        ...
        for (var name : limelightPoseEstimationList) megaTagSuccess |= updateVisionMegaTag2(name);
    ...
    private boolean updateVisionMegaTag2(String limelightName) {
        ...
        if (Math.abs(drive.getYawVelocityRadPerSec()) > (4 * Math.PI)) return false;

        // final double yaw_degrees = StateMachine.odometryAndVision.getRotation().getDegrees();
        final double yaw_degrees =
                StateMachine.odometryAndVision.getOdometryPose().getRotation().getDegrees();

        LimelightHelpers.SetRobotOrientation(limelightName, yaw_degrees, 0, 0, 0, 0, 0);
        LimelightHelpers.PoseEstimate mt2 = LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(limelightName);

        if (mt2.tagCount == 0) return false;
        ...
        // NOTE: we use MegaTag2 with a gyro-provided rotation, so trust the estimate's rotation fully instead of
        // calculating a deviation
        final double rotationStandardDev = 9999999;

        Logger.recordOutput("Vision/" + limelightName + "/Pose", mt2.pose);
        // StateMachine.odometryAndVision.addVisionObservation(new VisionObservation(mt2.pose, mt2.timestampSeconds,
        // stdDevs));
        StateMachine.odometryAndVision.addVisionObservation(new VisionObservation(
                mt2.pose, mt2.timestampSeconds, VecBuilder.fill(xyStandardDev, xyStandardDev, rotationStandardDev)));
